...
## 18. Финальная уборка перед сдачей (эта сессия)

Проект признан функционально завершённым (раздел 17 — все пункты ТЗ
закрыты, 80%+ покрытие подтверждено). По просьбе пользователя убрала
всё, что могло выглядеть незавершённым при просмотре репозитория:

- Удалены 12 бесхозных `.gitkeep`: 9 лежали в папках, которые давно
  заполнены реальным кодом (`config`, `controller`, `dto/request`,
  `dto/response`, `entity`, `exception`, `repository`, `service/impl`,
  и вложенный `test/service`); ещё 2 (`test/repository`,
  `test/resources`) — в папках-заготовках, которые так и не
  понадобились (repository-тесты и test-профиль не писали)
- Удалена папка `mapper/` целиком — MapStruct был подключён в
  `pom.xml`, но ни разу не использован (весь маппинг Entity↔DTO писали
  руками в `*ServiceImpl`, см. был TODO 6.8) — заодно убрана сама
  зависимость `mapstruct`/`mapstruct-processor` из `pom.xml`, чтобы не
  висела мёртвым грузом
- С диска (не из git — эти пути и так были в `.gitignore`, просто
  физически лежали в папке пользователя) убраны: `target/` (сборочный
  мусор Maven), `.idea/` (файлы IDE), и — важная находка — **вложенный
  git-репозиторий внутри `tools/.git/`**, судя по всему случайно
  созданный `git init` прямо в этой подпапке в какой-то момент работы;
  не был закоммичен в основной репозиторий (не gitlink), просто мусор
  на диске, но выглядел подозрительно при просмотре структуры проекта

После уборки: ни одной пустой папки, ни одной неиспользуемой
зависимости, `git status` чист. Дерево проекта соответствует тому, что
